...
import { 
  ...
  Calendar, Clock, MapPin, Car, UserCheck, Baby, User, ShieldCheck, Fingerprint // AJOUT D'ICONES
} from 'lucide-react';
    ...
    try {
      ...
      setResult({
        ...
        booking: {
          ...
          // RÉCUPÉRATION DES INFOS D'IDENTITÉ
          idType: b.passengers[0]?.id_type || 'Non spécifié',
          idNumber: b.passengers[0]?.id_number || 'Non spécifié',
          isChild: b.is_child,
  ...
  const handleAddLuggage = async () => {
    ...
    try {
      ...
      if (currentCalculation > 0) {
        await supabase.from('bookings').update({ status: 'ATTENTE_PAIEMENT' }).eq('id', result.booking.id);
      ...
      {result && result.booking && (
        <div className="space-y-4 animate-in slide-in-from-bottom-4 duration-500 text-left">
          <div className={`border-2 rounded-[2.5rem] p-5 bg-slate-900 shadow-2xl relative overflow-hidden ${result.valid ? 'border-emerald-500' : 'border-amber-500'}`}>
            ...
            {/* INFOS PASSAGER ET IDENTITÉ */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div className={`p-4 rounded-2xl border flex flex-col justify-center ${result.booking.isChild ? 'bg-blue-500/10 border-blue-500/20' : 'bg-slate-800/30 border-slate-700/50'}`}>
                    <div className="flex items-center gap-3">
                       ...
                       <div>
                          <Label className="text-[8px] font-black uppercase text-slate-500">
                            {result.booking.isChild ? "Passager Enfant" : "Nom du passager"}
                          </Label>
                ...
                <div className="p-4 rounded-2xl border border-slate-700/50 bg-slate-800/30 flex flex-col justify-center">
                    <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-full flex items-center justify-center border border-slate-700 bg-slate-900 text-emerald-500">
                            <ShieldCheck size={20} />
                        </div>
                        <div className="min-w-0">
                            <Label className="text-[8px] font-black uppercase text-slate-500">
                                {result.booking.isChild ? "Pièce Tuteur" : "Pièce d'identité"}
                            </Label>
                            <p className="text-xs font-black text-white uppercase truncate">{result.booking.idType}</p>
                            <p className="text-[10px] font-mono text-emerald-400 flex items-center gap-1 mt-0.5">
                                <Fingerprint size={10} /> {result.booking.idNumber}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-slate-950/50 p-5 rounded-[2rem] border border-slate-800 mb-6">
                <div className="flex items-center justify-between gap-4 mb-6 text-center">
                   ...
                   <div className="flex flex-col items-center px-4">
                      <div className="flex items-center gap-2 text-primary opacity-30">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 py-4 border-t border-slate-800/50">
            ...
            <div className="bg-slate-950 p-4 rounded-[2rem] border-2 border-slate-800 mb-6 shadow-inner">
                <div className="flex justify-between items-center mb-4">
                    <h4 className="text-[9px] font-black uppercase text-slate-400 flex items-center gap-2"><Scale size={14} className="text-primary"/> Nouveau Bagage / Pesée</h4>
                    <Badge variant="outline" className="text-[7px] border-emerald-500/30 text-emerald-400">{result.booking.freeWeight}kg Offerts</Badge>
                </div>
                <div className="space-y-4">
                    <div className="flex gap-2">
                        ...
                        <Input type="number" placeholder="0 KG" value={weightInput} onChange={e => setWeightInput(e.target.value)} className="w-20 h-12 rounded-xl border-none bg-slate-900 text-white font-black text-center" />
                    </div>
                ...
                <div className="bg-slate-800 p-5 rounded-[2rem] border border-slate-700 mb-6 shadow-2xl">
                    ...
                    {canCollectMoney ? (
                        ...
                        <div className="p-3 bg-amber-500/10 rounded-xl border border-amber-500/20 flex items-center gap-2">
                            ...
                            <p className="text-[9px] font-bold text-amber-500 uppercase italic text-center">Paiement requis au guichet principal</p>
                        </div>
                    )}
                </div>
            )}
