    ...
    <div className="max-w-2xl mx-auto w-full p-4 pb-20 space-y-8 text-left">
      ...
      <Tabs defaultValue="bus" className="w-full space-y-8">
        <TabsList className="bg-slate-100 p-1.5 rounded-2xl h-14 flex w-full">
          <TabsTrigger value="bus" className="flex-1 rounded-xl font-bold transition-all"><Package className="h-4 w-4 mr-2"/> Bus / Bateau (Forfait)</TabsTrigger>
          <TabsTrigger value="train" className="flex-1 rounded-xl font-bold transition-all"><Scale className="h-4 w-4 mr-2"/> Train / Bateau (Poids)</TabsTrigger>
        </TabsList>

        {/* --- MODE FORFAIT (Articles) --- */}
        <TabsContent value="bus" className="space-y-10 focus-visible:outline-none">
          <div className={`border-2 rounded-[2.5rem] p-8 shadow-xl transition-all ${editingId ? 'bg-orange-50/20 border-orange-200' : 'bg-white border-slate-100'}`}>
            <h3 className="text-sm font-black uppercase mb-6 flex items-center gap-2 text-slate-500">
                ...
                {editingId ? "Modifier le tarif" : "Nouveau forfait (Valise, Glacière, Sac)"}
            </h3>
            
            <div className="grid grid-cols-1 sm:grid-cols-12 gap-5">
              <div className="sm:col-span-7 space-y-2">
                <Label className="text-[10px] font-bold uppercase text-slate-400 ml-1">Type de bagage</Label>
                <Input placeholder="Ex: Glacière moyenne, Sac de voyage..." className="h-14 rounded-2xl border-2 border-slate-100 bg-white px-5 text-base" value={newItem.label} onChange={e => setNewItem({...newItem, label: e.target.value})} />
              </div>
              <div className="sm:col-span-3 space-y-2">
                <Label className="text-[10px] font-bold uppercase text-slate-400 ml-1">Prix (F)</Label>
                <Input type="number" className="h-14 rounded-2xl border-2 border-slate-100 bg-white px-5 font-black text-lg text-primary" value={newItem.price} onChange={e => setNewItem({...newItem, price: e.target.value})} />
              </div>
              <div className="sm:col-span-2 flex items-end">
                <Button onClick={handleUpsertBusItem} disabled={saving} className={`w-full h-14 rounded-2xl font-bold shadow-lg ${editingId ? 'bg-orange-500 hover:bg-orange-600' : 'bg-slate-900'}`}>
          ...
          <div className="space-y-3">
             {busItems.map((item) => (
                <div key={item.id} className={`flex items-center justify-between p-6 rounded-[2rem] border transition-all group ${editingId === item.id ? 'bg-orange-50 border-orange-200' : 'bg-white border-slate-100 hover:shadow-lg'}`}>
                  <div className="flex items-center gap-4 text-left">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-black text-orange-600 uppercase tracking-widest">{item.price.toLocaleString()} FCFA</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <Button variant="ghost" size="icon" onClick={() => startEdit(item)} className="text-slate-300 hover:text-orange-500 hover:bg-orange-50 rounded-full"><Edit2 size={18} /></Button>
                    <Button variant="ghost" size="icon" onClick={() => { if(confirm('Supprimer ?')) supabase.from('company_luggage_settings').delete().eq('id', item.id).then(()=>loadSettings()) }} className="text-slate-300 hover:text-red-600 hover:bg-red-50 rounded-full"><Trash2 size={18} /></Button>
                  </div>
                </div>
              ))}
          </div>
        ...
        <TabsContent value="train" className="focus-visible:outline-none">
          <div className="bg-white border-2 border-slate-50 rounded-[3rem] p-10 shadow-2xl shadow-slate-100/50 space-y-10 w-full">
            <div className="text-center">
               ...
               <h3 className="text-2xl font-black text-slate-900">Politique de Pesée</h3>
               <p className="text-sm text-muted-foreground font-medium italic text-center">Idéal pour le Train (SETRAG) et les Navires de mer</p>
            </div>

            <div className="space-y-8 text-left">
              <div className="space-y-3">
                <Label className="font-black text-[10px] uppercase text-slate-400 ml-4">Franchise (Poids gratuit par personne)</Label>
                <div className="relative">
                   <Input type="number" value={trainSettings.free_limit} onChange={e => setTrainSettings({...trainSettings, free_limit: parseFloat(e.target.value)})} className="h-20 pl-8 pr-20 font-black text-4xl rounded-[2rem] border-none bg-slate-50 focus:bg-white shadow-inner transition-all" />
                   <span className="absolute right-8 top-1/2 -translate-y-1/2 font-black text-slate-300 text-2xl">KG</span>
                </div>
              </div>

              <div className="space-y-3">
                <Label className="font-black text-[10px] uppercase text-slate-400 ml-4">Prix du surplus par KG</Label>
                <div className="relative">
                   <Input type="number" value={trainSettings.price_per_kg} onChange={e => setTrainSettings({...trainSettings, price_per_kg: parseFloat(e.target.value)})} className="h-20 pl-8 pr-32 font-black text-4xl rounded-[2rem] border-none bg-slate-50 focus:bg-white shadow-inner transition-all" />
