...
export default function AgencyUsers() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const { data: res, error } = await supabase.rpc('create_agency_staff', {
      p_email: email.trim().toLowerCase(), p_password: password, p_first_name: firstName, p_last_name: lastName, p_phone: phone, p_role: roleToAssign, p_company_id: user?.companyId
    });
    if (error || !res?.success) { toast.error(res?.error || "Erreur"); setSaving(false); return; }
