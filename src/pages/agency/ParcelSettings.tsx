...
import { Package, Truck, Plus, Trash2, RefreshCw, Calculator, Settings2, Save } from 'lucide-react';
    ...
    <div className="max-w-2xl mx-auto w-full p-4 pb-20 space-y-8 text-left">
      ...
      <header className="flex items-center gap-4 bg-white p-6 rounded-[2rem] border shadow-sm w-full">
        ...
        <div>
          ...
          <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">Grille officielle des colis</p>
        </div>
      </header>
      ...
      <Tabs defaultValue="flat" className="w-full space-y-8">
        ...
        {/* CARTE DE SAISIE - ESPACES AGRANDIS ET LISIBLES */}
        <div className="bg-white border-2 border-slate-100 rounded-[2.5rem] p-8 shadow-xl shadow-slate-100/50 w-full">
          <h3 className="text-sm font-black uppercase mb-6 flex items-center gap-2 text-slate-400">
            <Plus className="h-4 w-4" /> Configurer un nouveau prix
          </h3>
          
          <TabsContent value="flat" className="m-0 space-y-6 focus-visible:outline-none">
             <div className="grid grid-cols-1 sm:grid-cols-12 gap-5">
                <div className="sm:col-span-7 space-y-2">
                    <Label className="text-[10px] font-black uppercase text-slate-400 ml-1">Type de colis</Label>
                    <Input 
                      placeholder="Ex: Enveloppe, Carton, Malle..." 
                      className="h-14 rounded-2xl border-2 border-slate-100 bg-white px-5 text-base font-medium focus:border-emerald-500 transition-all" 
                      value={newItem.label} 
                ...
                <div className="sm:col-span-3 space-y-2">
                    ...
                    <Input 
                      type="number" 
                      className="h-14 rounded-2xl border-2 border-slate-100 bg-white px-5 font-black text-lg text-emerald-600 focus:border-emerald-500 transition-all" 
                      value={newItem.price} 
          ...
          <TabsContent value="weight" className="m-0 space-y-6 focus-visible:outline-none">
             <div className="grid grid-cols-1 sm:grid-cols-12 gap-5">
                <div className="sm:col-span-7 space-y-2">
                    ...
                    <Input 
                      placeholder="Ex: Sac de Farine, Matériel BTP..." 
                      className="h-14 rounded-2xl border-2 border-slate-100 bg-white px-5 text-base font-medium focus:border-blue-500 transition-all" 
                      value={newItem.label} 
                      onChange={e => setNewItem({...newItem, label: e.target.value})} 
                    />
                </div>
                <div className="sm:col-span-3 space-y-2">
                    ...
                    <Input 
                      type="number" 
                      className="h-14 rounded-2xl border-2 border-slate-100 bg-white px-5 font-black text-lg text-blue-600 focus:border-blue-500 transition-all" 
                      value={newItem.price} 
                      onChange={e => setNewItem({...newItem, price: e.target.value})} 
                    />
                </div>
        ...
        <div className="space-y-4 w-full">
          <h3 className="font-black text-[10px] uppercase tracking-widest text-slate-400 px-6">Tarifs actifs</h3>
